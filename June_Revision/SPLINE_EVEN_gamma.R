...
peak_P_low <- rep(NA, length(even.sample.list))
peak_P_high <- rep(NA, length(even.sample.list))
peak_T_inCI <- rep(NA, length(even.sample.list))
...
for(i in 1:length(even.sample.list)){
  ...
  kspline_I <- ((R_t_k +
               kspline$y[-1] *
           (pars[even.sample.list[[i]]$pars_id[1],]$omega_a+with(pars[even.sample.list[[i]]$pars_id[1],], b_func(t=t_spline, g=g, s=s, psi=psi, f=f)) )  )/
                pars[even.sample.list[[i]]$pars_id[1],]$gamma)
...
peaks_even_df <- data.frame(peak_P,peak_P_low, peak_P_high, peak_T_inCI, peak_T,peak_gamma,pars_id,type)
peaks_even_df$CIsize <- peaks_even_df$peak_P_high-peaks_even_df$peak_P_low
peaks_even_df$dist <- abs(peaks_even_df$peak_P-peaks_even_df$peak_T)
...
ggplot(D.long, aes(x=variable, y=value, fill=variable))+ylab("Proportion Successful Sampling Bouts")+xlab("Sampling Time Point")+
  geom_boxplot(outlier.shape = 20)+scale_fill_brewer(NULL,palette="Paired",labels=c(TeX("True $(\\iota)$"), TeX("Predicted $(\\hat{\\iota})$"),TeX("Null ($1/\\gamma$)"),'Null (random)'))+
