...
model {
  x ~ binomial(n,  g*exp (-s* pow (cos (-psi + pi()*t*2/52), 2) ) ); // likelihood
}"

# 0.5 pi for Hypsig data
ra_func.stan.week =
  "data {
  int<lower=0> N;   // number of data items
  int<lower=0> n[N];    // number of trials in i
...
parameters {
  ...
  real<lower=0, upper=0.5*pi()> phi;    // phase shift 
  real<lower=C1, upper=1+C1/exp(a)> C2;    // verticle shift 
}
model {
  x ~ binomial(n,  C2 - C1*exp (-a* pow (cos (-phi + pi()*t*2/52), 2) ) ); // likelihood
}"
