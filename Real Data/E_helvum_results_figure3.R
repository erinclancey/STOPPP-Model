# R code to estimate the birth function
# R code to fit r_A(t) with kernal smoothing
# R code to predict iota(t) 
...
ggplot(agg_lac, aes(x=week, y=lac_Prop))+geom_point(color="black",fill="black", size = 2, shape = 21)+
  scale_x_continuous(breaks=seq(-4,48,2), limits=c(-4,48))+
...
# Rescale g
b_func_helvum <- function(t) exp(-mode.preg[2]*cos(pi*1/52*t-mode.preg[3])^2)
int=integrate(b_func_helvum, 0, 52)$value
g=1/(313)*52 / int
...
# add corresponding per capita birth rates to dataframe for analyses and plotting
EBOVagg_cases$b <- b_func(t=EBOVagg_cases$Week, g=g, s=mode.preg[2], psi=mode.preg[3], f=1/52)

# Predict r_A(t) and iota(t) with kernal smoothing
df <- EBOVagg_cases
kspline <- ksmooth(df$Week, df$EBOVPos_prop, "normal", bandwidth = 15, n.points=500)
...
# Find the peak and 90th quantile
...
# Plot the results shown in Figure 3
A <- ggplot() +
  ...
  scale_y_continuous(limits = c(0, 1)) + scale_x_continuous(breaks=seq(-2,48,2), limits=c(-2,48))+
...
B <- ggplot() +
  ...
  theme(axis.text = element_text(size = 10), axis.title = element_text(size = 13))+
  theme(legend.position = "right")+
  scale_y_continuous(limits = c(0, 0.25)) +  scale_x_continuous(breaks=seq(-2,48,2), limits=c(-2,48))+
