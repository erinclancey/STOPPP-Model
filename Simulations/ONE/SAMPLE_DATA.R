...
even.sample.list <- c(sample_daily,sample_weekly,sample_monthly,sample_bimonthly)    
...
#   sample_42_week[[i]] <- df
# }
# # sample_42_month <- list()
# # for(i in 1:length(sample_daily) ){
# #   df <- sample_daily[[i]]
# #   index <- seq(1:27) #two week periods in timeframe
# #   month <- sample(index,3,replace=FALSE)
# #   day <- month*14
# #   days <- c(sapply(day,function(x){seq(x,x+13)}))
# #   df_w <- df[days,]
# #   df <- df_w
# #   df <- df[order(df$t),]
# #   df$pars_id <- rep(i, sample_days)
# #   df$type <- rep("42_month", sample_days)
# #   sample_42_month[[i]] <- df
# # }
# cluster.sample.list <- c(sample_42_random,sample_42_3_day,sample_42_week)
# 
# 
