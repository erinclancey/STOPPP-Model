# R code to Sample from the 300 epidemic population time series

n_trials=20
n_sets=length(data.tau.list.1_365)
############Daily Samples
sample_daily <- list()
for(i in 1:n_sets){
  t <- data.tau.list.1_365[[i]]$t
  I_sample <- vector()
  R_a_sample <- vector()
  for(k in 1:length(t)) {
    I_sample[k] <- with(data.tau.list.1_365[[i]], rbinom(1, n_trials, I[k]/N[k]))
    R_a_sample[k] <- with(data.tau.list.1_365[[i]], rbinom(1, n_trials, R_a[k]/N[k]))
  }
