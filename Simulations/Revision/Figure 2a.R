...
kspline_I <- ((R_t_k +
                 kspline$y[-1] *
                 (pars[sample.list[[set]]$pars_id[1],]$omega_a+with(pars[set,], 
                                                    b_func(t=seq(2,365,1), g=g, s=s, psi=psi, f=f)))  )/
                pars[sample.list[[set]]$pars_id[1],]$gamma)
...
A <- ggplot() +
  ...
  geom_line(data = data.frame(Time = data.tau.list.1_365[[1]]$t[-1], 
                              Value = data.tau.list.1_365[[set]]$R_a[-1] / data.tau.list.1_365[[set]]$N[-1]),
...
######################################
B <- ggplot() +
  geom_line(data = data.frame(Time = data.tau.list.1_365[[set]]$t[-1], 
                              Value = data.tau.list.1_365[[set]]$I[-1] / data.tau.list.1_365[[set]]$N[-1]),
  ...
  geom_vline(xintercept = pars[set,]$peak_I_1, linetype=3, size=1)
