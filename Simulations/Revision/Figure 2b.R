...
for(i in 1:length(sample.list)){
  df <- sample.list[[i]]
  kspline <- ksmooth(df$t, df$R_a_sample/n_trials, "normal", bandwidth=90, n.points=500)
  ...
  peak_T[i] <- pars[sample.list[[i]]$pars_id[1],]$peak_I_2
...
for(j in 1:10){
  ...
  type <- vector()
  for(i in 1:length(sample.list)){
    data=data.tau.list.366_730[[sample.list[[i]]$pars_id[1]]]
    type[i]=peaks_df$type[i]
    Y_random=data[sample(nrow(data), 1, replace = FALSE), ] 
    Y_true <- subset(data, t==pars[sample.list[[i]]$pars_id[1],]$peak_I_2) 
