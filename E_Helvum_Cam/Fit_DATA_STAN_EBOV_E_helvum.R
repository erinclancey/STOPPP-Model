...
plotpostre = ggplot(ra_posterior, aes(x=a_post)) +
...
brate = b_func (t=EBVagg_cases$Week[-1], f=1/52,g=0.204,
                s=11.2,
                psi=-1 )
...
kspline <- ksmooth(EBVagg_cases$Week, EBVagg_cases$EBVPos_prop, "box", 
                   bandwidth = 30, n.points=length(EBVagg_cases$Week) )
...
Ispline <- ksmooth(t_spline, I_new_k , "box", 
                   bandwidth = 50, n.points=length(t_spline) )
lines(t_spline, Ispline$y, col="blue")
plot(t_spline, I_new_k , col="blue")

MaxI <- data.frame(t_spline, I_new_k)
#peak occurs at week 37
...
ggplot(EBVagg_cases, aes(x=Week, y=EBVPos_prop))+geom_point()+scale_x_continuous(breaks=seq(0,51,2))+
  ...
  stat_function(fun=function(t=Week, a=mode[1],
                             C1=mode[2],
                             C2=mode[3],
                             phi=mode[4]) -C1*exp(-a*cos(pi/52*t-phi)^2)+C2, color="red")
